...
% Was a testing model used? 																No
% Should a testing model have been used? 													...
% Why was testing important for this project?												Testing helped ensure a consistent interface between classes and functions
% When did the testing start in dev cycle?  												During the later part of the project lifetime
% Would the XP test strategy have been suitable?  											...
% Are there machine learning specific test strategies which could have been followed?  		...
...
% Hard: it is not always easy to determine whether particular functions return sensible values, for example whether a sensible subset of restaurants are filtered out for being too far. As such, the test suite of the project focuses on invoking such functions and inspecting the returned type.

% Discuss why continuous integration was particularly important for the project:
% Jenkins, the continuous integration environment for this project, was useful for automatically running test cases after each comming to the source code. The integration of Cobertura provided coverage reports that helped indicating the parts of the project that required additional testing.
% The unit tests were set up to run on Jenkins, a continuous integration environment. Jenkins was set up to push notifications to the team's communication channel whenever a test case would fail. This enabled early notification of issues, allowing the team to patch up bugs early on. Jenkins also provided coverage reports for the source files, indicating the percentage of classes, conditionals and lines covered.
...
% Coverage percentage which, although different for classes, lines and conditionals, stands above 50
...
% Above is Dom / Joseph's Notes
% Below is general structure of how I see the section coming together

% Lead in with a general statement about why testing is imporant even when the project itself is research focused (R\&D) project.

% General outline of when testing started and what the test process was for the project - continous integration / how it was decided to test the aspects that we did end up testing. Explain why testing started when it did (due to nature of project and tests which would have been required to be rewritten). Could even say the team considered a TDD strategy but this was decided against for the above reasons. Explain how the decision was made instead to take the approach that we did as the team believed it would be suitable for the project. Talk about why the testing we did was good.
...
Primiarly the testing comprised of the creation of a suite of unit tests to ensure that each alteration to the source code did not break the execution / build of the system. Such unit tests were setup to run with the Jenkins continous integration environment with was configured to push notifications to the team's communication channel (the Slack collaboration tool) in the event of test case failure. % Find academic / industry reference for why this setup is standard practice and its benefits
This allowed for early notification of issue / broken builds allowing the team to ammend errors / patch bugs / etc. as soon as possible. % Talk about why this is industry practice via references.
...
% Lead into this better but...
The Cobertura extension to Jenkins provided the team with coverage reports for the source files to indicate the percentage of classes, conditionals and lines covered. Such reports were then used to help identify aspects of the system where additional testing was required. % Reference and talk about why code coverage reports are beneficial in industry and how they can be used to improve testing as we did - reflect that this was a good thing the team did and ultimately helped drive and improve the test process

% Continue overview of the test process as described above
...
% Recommend (see what I did there) what other testing could have been integrated (acceptance, etc.) and suggest a possible strategy for this. 
...
% NOTE:         Temp notes / thoughts
% ################# Comment Log ####################
% Pair programming was benefical to the project and developers
% Yes... it was
% This talks about the practice in general and the specific incidents and why they were beneficial to us
% Also mini reflection on front end technology choice (we had all used Node from IS project though embedded JS was new)
% ##################################################

% This had a positive effect for the padawan developers of the unfamilar aspects of the system and getting them to a confidence level where they felt better
% prepared to jump in and figure things out for themselves - back up this as a legit practive via references.
